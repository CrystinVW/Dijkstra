    ...
    def dijkstra(self, start):
        ...
        while todo:
            ...
            for node in todo:
                ...
                if node.distance < node.previous:
                    node.previous = node
            ...
            for edge in min_node.edges:
                ...
                temp_node = edge.weight + min_node.distance
                ''' If the new distance is less than the previous
                    distance '''
                # print(temp_node)
                if temp_node < edge.to_node.distance:
                    edge.to_node.distance = temp_node
        ...
        if not track_prev:
            ...
            for node in self.__nodes:
                ...
                result.append([node.distance, node.previous[:-1]])
            ''' For each node, create a list where the first element
